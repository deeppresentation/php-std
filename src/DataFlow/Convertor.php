...
class Convertor
{
    ...
    private static function convert_sub_value($subValue, $convertAction)
    {
        ...
        switch ($convertType)
        {
            case 'capitalize': $result = self::capitalize($subValue, $args); break;
            case 'convert-table': $result = self::convert_by_table($subValue,  $args, $nextConvertCustom); break;
            case 'associative': $result = self::associative($subValue, $args); break;
            case 'set-custom-value': $result = self::set_custom_value($subValue, $args); break;
        ...
        {
            case 'result': $nextConvert = $result; break;
            case 'input': $nextConvert = $subValue; break;
            case 'custom': $nextConvert = $nextConvertCustom; break;     
        }
        return array(
            'value' => $result ?? '',
            'next' => $nextConvert
    ...
    public static function process($value, $convertArgs)
    {
        if (isset($value) && isset($convertArgs))
        {
            ...
            if (isset($convertActions))
            {
                ...
                switch ($resultType)
                {
                    case 'string': return Arr::as_string($result);
                    case 'key-val': return  array(key(Arr::as_array($result)) => Arr::as_string($result));
                    case 'array': return Arr::as_array($result);
